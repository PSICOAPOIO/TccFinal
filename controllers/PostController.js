...
module.exports = class PostController{
    static async showPost(req,res){
        ...
        consult.forEach((item)=>{
           ...
           // item.comentarios=comentariosData.map((result) => result.get({where: {postId:item.id}}))
            //item.comentarios=comentariosData.map((result) => result.get(Post.findAll({where: {id:item.id}})))
            //console.log(item.comentarios=comentariosData.map((result) => result.get({where:{id: id}})))
    // console.log(item)
            posts.push(item)
